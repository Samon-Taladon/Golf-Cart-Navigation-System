#!/usr/bin/env python3
"""Prepare logs12 waypoints for enhanced_gnss_publisher.py and purepursuit.py.

Input:
    waypoints_clean.csv with columns: lat,lon,speed,heading,fix_quality

Outputs:
    waypoints_utmlog12.csv              lat/lon converted to UTM x/y
    path_smoothlog12.csv                smoothed and distance-filtered x/y path
    path_smoothlog12_resampled_1m.csv   purepursuit-compatible x,y path
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--utm-output",
        type=Path,
        default=script_dir / "waypoints_utmlog12.csv",
        help="Output CSV containing x,y plus original waypoint metadata.",
    )
    parser.add_argument(
        "--smooth-output",
        type=Path,
        default=script_dir / "path_smoothlog12.csv",
        help="Output smoothed path CSV with x,y columns.",
    )
    parser.add_argument(
        "--resampled-output",
        type=Path,
        default=script_dir / "path_smoothlog12_resampled_1m.csv",
