...
#         self._rtcm_bytes = 0
#         self._gga_sent_count = 0
#         self._last_ntrip_stats_log = 0.0
#         self._last_logged_rtcm_bytes = 0
#         self._last_rtcm_time = 0.0
#         self._seen_rtcm_chunk = False
...
#                 first_payload = self._read_ntrip_response(sock)
...
#                 if first_payload:
#                     self._write_rtcm(first_payload)
...
#                         else:
#                             raise ConnectionError("NTRIP socket closed by caster")
...
#                             try:
#                                 sock.sendall((gga + "\r\n").encode())
#                                 self._gga_sent_count += 1
#                                 last_send = now
#                             except Exception:
#                                 break

#                     if now - self._last_rtcm_time > NTRIP_RTCM_STALL_TIMEOUT:
#                         raise TimeoutError(
#                             f"No RTCM data for {NTRIP_RTCM_STALL_TIMEOUT:.0f}s"
#                         )

#                     self._log_ntrip_stats()
...
#     def _read_ntrip_response(self, sock):
#         sock.settimeout(NTRIP_CONNECT_TIMEOUT)
#         response = b''
...
#                 break
#             response += chunk
#             if response.startswith(b'ICY 200'):
#                 break

#         header, sep, payload = response.partition(b'\r\n\r\n')
#         if not sep and response.startswith(b'ICY 200'):
#             header, _line_sep, payload = response.partition(b'\r\n')

#         header_text = header.decode('ascii', errors='replace')
#         first_line = header_text.splitlines()[0] if header_text else ''
#         if '200' not in first_line:
#             raise RuntimeError(f"NTRIP rejected request: {first_line or header_text[:120]}")

#         self.get_logger().info(f"🛰 NTRIP response: {first_line}")
#         return payload

#     def _write_rtcm(self, data):
#         if not self._seen_rtcm_chunk:
#             self._seen_rtcm_chunk = True
#             prefix = data[:12].hex(' ')
#             if not data.startswith(b'\xd3'):
#                 self.get_logger().warn(
#                     f"First NTRIP payload does not start with RTCM preamble D3: {prefix}"
#                 )
#             else:
#                 self.get_logger().info(f"First RTCM payload starts with: {prefix}")
...
#         now = time.time()
#         if now - self._last_ntrip_stats_log < 5.0:
#             return
#         self._last_ntrip_stats_log = now
#         rtcm_delta = self._rtcm_bytes - self._last_logged_rtcm_bytes
#         self._last_logged_rtcm_bytes = self._rtcm_bytes
#         self.get_logger().info(
#             f"📶 NTRIP stats: RTCM {self._rtcm_bytes} bytes "
#             f"(+{rtcm_delta}) -> u-blox, "
#             f"GGA sent {self._gga_sent_count} times",
#         )
...
# # -------- ปรับให้ตรงกับ waypoint logger --------
# GGA_SEND_INTERVAL     = 1.0   # VRS casters usually expect rover GGA at about 1 Hz.
# ALLOWED_FIX           = [4, 5]  # RTK Fixed ✅ + RTK Float 🔶 (เหมือน waypoint logger)
# MIN_SPEED_FOR_HEADING = 0.5   # m/s — ตรงกับ MIN_SPEED ของ waypoint logger
# # -----------------------------------------------
...
#     #  NTRIP thread
#     #  ⭐ ปรับ logic การส่ง GGA ให้ตรงกับ waypoint logger
#     #     - ส่ง GPGGA-compatible sentence ให้ VRS caster
#     #     - ส่งทุก GGA_SEND_INTERVAL
#     # ══════════════════════════════════════════════════════════════════════════
#     def _ntrip_thread(self):
#         while True:
...
#             try:
#                 local_addr = self._wait_for_network()
#                 initial_gga = self._wait_for_initial_gga()
#                 self.get_logger().info(
#                     f"🛰 Connecting NTRIP via active network ({local_addr})..."
#                 )
#                 sock = self._open_ntrip_socket()
...
#                 # VRS casters often need the rover position before streaming RTCM.
#                 request = (
#                     f"GET /{NTRIP_MOUNTPOINT} HTTP/1.0\r\n"
#                     f"User-Agent: NTRIP ROS2Client/1.0\r\n"
#                     f"Accept: */*\r\n"
#                     f"Ntrip-GGA: {initial_gga}\r\n"
#                     f"Authorization: Basic {auth}\r\n\r\n"
#                 )
#                 sock.sendall(request.encode())
#                 first_payload = self._read_ntrip_response(sock)
...
#                         if gga:
#                             try:
#                                 gga = self._format_gga_for_ntrip(gga)
#                                 sock.sendall((gga + "\r\n").encode())
#                                 self._gga_sent_count += 1
#                                 last_send = now
#                             except Exception:
...
#     def _wait_for_initial_gga(self):
#         deadline = time.time() + 15.0
#         while rclpy.ok() and time.time() < deadline:
#             with self._gga_lock:
#                 gga = self._last_gga_sentence
#             if gga:
#                 return self._format_gga_for_ntrip(gga)
#             time.sleep(0.1)
#         raise TimeoutError("No GGA sentence available for NTRIP request")

#     def _format_gga_for_ntrip(self, sentence):
...
class GnssPosePublisher(Node):
    ...
    def _ntrip_thread(self):
        while True:
            ...
            # ---------- ตรวจสอบ Header ----------
                ...
                self.get_logger().info("========== NTRIP RESPONSE ==========")
                self.get_logger().info(header.decode(errors="ignore"))

                if b"200 OK" not in header and b"ICY 200 OK" not in header:
                    raise RuntimeError(
                        f"NTRIP Login Failed :\n{header.decode(errors='ignore')}"
                    )

                self.get_logger().info("✅ NTRIP Login Success")
                ...
                self.get_logger().warn(
                    f"Reconnect in {NTRIP_RETRY_INTERVAL} sec..."
                )
    ...
    def handle_nmea_line(self, line):
        ...
        try:
            ...
            # ── Heading จาก u-blox RMC true_course เท่านั้น ────────────────
            yaw = self.prev_yaw if self.prev_yaw is not None else 0.0
            yaw_source = 'fallback'
            ...
            # ถ้า u-blox ไม่ส่ง true_course ให้คง yaw ล่าสุดไว้ ไม่รับจาก IMU
            # และไม่คำนวณ heading เองจาก delta ของตำแหน่ง
