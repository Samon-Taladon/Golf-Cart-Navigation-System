...
class PurePursuitController(Node):
    ...
    def __init__(self):
        ...
        self.bicycle_model_steering_gain = float(
            self.get_parameter('bicycle_model_steering_gain').value
        )
        ...
        # Kinematic bicycle model dead-reckoning state
        self.bicycle_x = 0.0
        self.bicycle_y = 0.0
    ...
    def odom_callback(self, msg: Odometry) -> None:
        ...
        if new_rtk_state == 'FLOAT' and (
            ...
            self.bicycle_x   = seed_x
            self.bicycle_y   = seed_y
            self.bicycle_yaw = seed_yaw
            self.bicycle_active = True
            self.float_started_time = now
            self.bicycle_propagation_count = 0

            self.get_logger().info(
                f'RTK FLOAT: bicycle model seeded from last FIX → '
                f'x={seed_x:.3f}  y={seed_y:.3f}  yaw={math.degrees(seed_yaw):.1f}°'
            )

        # ── Transition: NO_FIX ครั้งแรก → seed bicycle model ────────────────
        if new_rtk_state == 'NO_FIX' and not self.bicycle_active:
            if self.last_known_good_x is not None:
                self.bicycle_x   = self.last_known_good_x
                self.bicycle_y   = self.last_known_good_y
                self.bicycle_yaw = self.last_known_good_yaw
                self.bicycle_active = True
                self.float_started_time = now
                self.bicycle_propagation_count = 0
                self.get_logger().warn(
                    f'RTK NO_FIX: bicycle model seeded from last_known_good → '
                    f'x={self.bicycle_x:.3f}  y={self.bicycle_y:.3f}  '
                    f'yaw={math.degrees(self.bicycle_yaw):.1f}°'
                )

        # ── Transition: FLOAT/NO_FIX → FIX หรือ DGPS ────────────────────────
        if new_rtk_state in ('FIX', 'DGPS') and self.bicycle_active:
            self.bicycle_active = False
            self.float_started_time = None
            self.bicycle_propagation_count = 0
            self.get_logger().info(
                f'RTK {new_rtk_state} (q={self.fix_quality}) at '
                f'x={msg_x:.3f}  y={msg_y:.3f}; bicycle model deactivated'
            )
        ...
        # ════════════════════════════════════════════════════════════════════
        #  FLOAT — dead-reckoning ด้วย kinematic bicycle model
        # ════════════════════════════════════════════════════════════════════
        if self.rtk_state == 'FLOAT':
            dt = self.control_period if self.last_odom_time is None else max(
            ...
            self.predict_position_bicycle(dt)
            self.bicycle_propagation_count += 1
            self.last_odom_time = now
            self.position_is_predicted  = True
            self.have_odom = True
            self.allow_waypoint_progress = self.bicycle_propagation_count >= 1
            ...
            if float_elapsed > self.rtk_float_timeout:
                self.throttled_warn(
                    f'FLOAT bicycle model active ({float_elapsed:.1f}s) | '
                    f'bicycle=({self.bicycle_x:.2f}, {self.bicycle_y:.2f}) | '
                    f'ublox_raw=({msg_x:.2f}, {msg_y:.2f}) [ignored]'
                )
            return

        # ════════════════════════════════════════════════════════════════════
        #  NO_FIX — bicycle model ถ้า active / หยุดรถถ้าไม่เคยมี FIX เลย
        # ════════════════════════════════════════════════════════════════════
        if self.rtk_state == 'NO_FIX':
            if self.bicycle_active:
                dt = self.control_period if self.last_odom_time is None else max(
                    now - self.last_odom_time, self.control_period
                )
                self.predict_position_bicycle(dt)
                ...
                self.throttled_warn(
                    f'NO_FIX bicycle model active ({no_fix_elapsed:.1f}s) | '
                    f'bicycle=({self.bicycle_x:.2f}, {self.bicycle_y:.2f}) | '
                    f'ublox_raw=({msg_x:.2f}, {msg_y:.2f}) [ignored]'
                )
                return
    ...
    def predict_position_bicycle(self, dt: float) -> None:
        ...
        # midpoint method: ประมาณ yaw กลาง dt เพื่อ accuracy ที่ดีขึ้นเมื่อเลี้ยว
        if self.wheelbase > 0.0:
            yaw_rate = (v / self.wheelbase) * math.tan(self.last_steering)
        else:
            yaw_rate = 0.0
        mid_yaw = self.normalize_angle(self.bicycle_yaw - yaw_rate * dt / 2.0)

        # คำนวณเฉพาะ XY
        self.bicycle_x += v * math.cos(mid_yaw) * dt
        self.bicycle_y += v * math.sin(mid_yaw) * dt

        self.current_x   = self.bicycle_x
        self.current_y   = self.bicycle_y
        self.current_yaw = self.bicycle_yaw  # yaw มาจาก ublox โดยตรง
    ...
    def control_loop(self) -> None:
        ...
        if self.is_float_timeout_exceeded(now):
            self.throttled_warn(
                f'RTK {self.rtk_state} bicycle model active: '
                f'pos=({self.current_x:.2f},{self.current_y:.2f}), '
                f'yaw={math.degrees(self.current_yaw):.1f} deg'
            )
        ...
        if self.bicycle_active:
            self.debug_bicycle(lookahead, steering_raw, steering)
        else:
            self.debug_log(lookahead, steering_raw, steering)
            if self.rtk_state == 'DGPS':
                self.throttled_warn(
    ...
    def is_float_timeout_exceeded(self, now: float) -> bool:
        """True when we have been in FLOAT/NO_FIX bicycle mode longer than rtk_float_timeout."""
        if self.float_started_time is None or not self.bicycle_active:
            return False
        return now - self.float_started_time > self.rtk_float_timeout

    def debug_bicycle(self, lookahead: float, steering_raw: float, steering: float) -> None:
        now = self.get_clock().now()
        if (now - self.last_debug_time).nanoseconds / 1e9 < self.debug_period:
            return
        self.last_debug_time = now
