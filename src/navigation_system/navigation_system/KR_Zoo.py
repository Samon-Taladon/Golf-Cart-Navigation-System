...
class UbloxDestinationBridge(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter('cmd_vel_topic', '/cmd_vel_2')
        ...
        self.latlon_to_utm = Transformer.from_crs(
            'epsg:4326', 'epsg:32647', always_xy=True
        )
        self.utm_to_latlon = Transformer.from_crs(
            'epsg:32647', 'epsg:4326', always_xy=True
        )
        ...
        self.get_logger().info(
            ...
            f'{self.odom_topic} -> {self.cmd_vel_topic}, '
    ...
    def set_destination_from_payload(self, data):
        ...
        dest_x, dest_y = self.latlon_to_utm.transform(lon, lat)
    ...
    def odom_callback(self, msg):
        ...
        self.current_position = {
            ...
            'y': y,
            'stamp': time.time(),
            'fix_quality': float(msg.pose.covariance[0]),
        }
        self.last_distance_m = self.calculate_current_distance()
        ...
        if now - self.last_status_sent_at >= 0.5:
            ...
            self.schedule_broadcast_status('odom_update')
    ...
    def calculate_current_distance(self):
        if not self.current_position or not self.destination:
            return None
        return math.hypot(
            self.destination['x'] - self.current_position['x'],
            self.destination['y'] - self.current_position['y'],
        )
