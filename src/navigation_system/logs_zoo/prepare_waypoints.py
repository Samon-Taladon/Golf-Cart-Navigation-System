#!/usr/bin/env python3
"""Prepare logs_zoo waypoints for enhanced_gnss_publisher.py and purepursuit.py.

Input:
    waypoints_clean.csv with columns: lat,lon,speed,heading,fix_quality

Outputs:
    waypoints_utmlogs_zoo.csv              lat/lon converted to UTM x/y
    path_smoothlogs_zoo.csv                smoothed and distance-filtered x/y path
    path_smoothlogs_zoo_resampled_1m.csv   purepursuit-compatible x,y path
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Convert logs_zoo lat/lon waypoints to UTM and Pure Pursuit path files."
    )
    parser.add_argument(
        "--input",
        type=Path,
    ...
    parser.add_argument(
        "--utm-output",
        type=Path,
        default=script_dir / "waypoints_utmlogs_zoo.csv",
        help="Output CSV containing x,y plus original waypoint metadata.",
    )
    parser.add_argument(
        "--smooth-output",
        type=Path,
        default=script_dir / "path_smoothlogs_zoo.csv",
        help="Output smoothed path CSV with x,y columns.",
    )
    parser.add_argument(
        "--resampled-output",
        type=Path,
        default=script_dir / "path_smoothlogs_zoo_resampled_1m.csv",
